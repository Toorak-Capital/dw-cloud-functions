...
def trigger_on_am_report(file_path, file_uri):
    ...
    if 'AM Consolidated' in file_path:

        formatted_date = extract_date(file_path)
        df = read_excel(file_uri, sheet_name = 'Toorak Master List_Bridge')
        write_parquet_by_date(df, 'AM-Securitization', 'originator_am_mapping', formatted_date)

    if 'Securitization' in file_path:

        formatted_date = extract_date(file_path)

        #materially modified loans
        materially_modified_loans = read_excel(file_uri, sheet_name = 'Materially Modified Loans')
        write_parquet_by_date(materially_modified_loans, 'AM-Securitization', 'materially_modified_loans', formatted_date)
        ...
        write_parquet_by_date(exclusion_list_60_deq, 'AM-Securitization', 'exclusion_list/deq_60_plus', formatted_date)
        write_parquet_by_date(exclusion_list_default_mortgage_loans, 'AM-Securitization', 'exclusion_list/default_mortgage_loans', formatted_date)
        ...
        write_parquet_by_date(funding_account_balance, 'AM-Securitization', 'funding_account_balance', formatted_date)


        #targeted rate data
        targeted_rate = read_excel(file_uri, sheet_name = 'Targeted Rate')
        write_parquet_by_date(targeted_rate, 'AM-Securitization', 'targeted_rate', formatted_date)
        ...
        write_parquet_by_date(trigger_rate, 'AM-Securitization', 'trigger_rate', formatted_date)
